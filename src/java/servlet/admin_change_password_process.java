...
@WebServlet(name = "admin_change_password_process", urlPatterns = {"/admin_change_password_process"})
public class admin_change_password_process extends HttpServlet {
    ...
    String password = request.getParameter("password");
                ...
                if (rsConfirm.next()) 
                {
                    ...
                    {
                        String stAdd = "update admin set password = ?";
                        PreparedStatement prpAdd = con.prepareStatement(stAdd);
                
                        prpAdd.setString(1,new_password);
                        prpAdd.execute();

                        String sqlLog = "insert into log(acct_type, username, date, change_type, description,time) values ('admin', '"+username+"','"+cdate+"','Update Details','Changed Password','"+ct+"')";
                ...
                {
                    session.setAttribute("edit_message", "&#128712 Error Occured Contact Admin "+e);
                    response.sendRedirect("admin_change_password.jsp");
                }
