 ...
 * @author admin
 */
@WebServlet(name = "edit_personnel_process", urlPatterns = {"/edit_personnel_process"})
public class edit_personnel_process extends HttpServlet {
    ...
    String password = request.getParameter("password");
            ...
            if (age > -1)
            {
                ...
                ResultSet rsConfirm;
                String stAdd = "update receptionist set name = ?, gender = ?, address = ?, phone = ?, tphone = ?, status = ?, email = ?,  birthday = ?, date_updated = ?, acct_status = ? where count ='"+ session.getAttribute("personnel_id")+"'";
                ...
                if (rsConfirm.next()) 
                {
                    ...
                    prpAdd.setString(10,acct_status);
                    prpAdd.execute();

                    String sqlLog = "insert into log(acct_type, username, date, change_type, description,time) values ('admin', '"+username+"','"+cdate+"','Update Details','Edited Personel "+name+"','"+ct+"')";
                ...
                else
                {
                    ...
                    response.sendRedirect("admin_view_personnel_edit.jsp");
                }
                }
                catch (Exception e)
                {
